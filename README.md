...
[![OpenAI](https://img.shields.io/badge/OpenAI-Vision-orange)](https://openai.com/)
...
**문제 정의**: 집안 물건을 찾느라 시간을 낭비하고, 이미 있는 물건을 중복 구매하는 일이 빈번함

**해결책**: 사진 한 장 업로드 → AI가 자동으로 물건을 인식하고 분류해서 체계적으로 관리

**핵심 가치**: AI 기술을 활용한 실용적인 문제 해결과 사용자 경험 개선

### 주요 기능

- **AI 이미지 분석**: OpenAI Vision API로 사진 속 물건을 자동 인식하고 카테고리/태그를 생성
- **위치별 관리**: 방과 서랍 정보로 물건 위치 체계적 관리
- **개인 데이터 보호**: 사용자별 완전 분리된 개인 물건 관리
...
- **이중 AI 제공자**: OpenAI GPT-4V 우선, 실패 시 Anthropic Claude로 자동 폴백
...
- **표준 카테고리 체계**: 10개 주요 카테고리, 65개 세부 분류 정의
...
- **AI 통합**: OpenAI Vision API 성공적 통합 및 프롬프트 엔지니어링 경험
...
- **환경 격리 우선**: 프로젝트 시작과 동시에 로컬/테스트/프로덕션 완전 분리하여 나중의 대규모 리팩토링과 데이터 오염 사고
  방지
